...
function query(html){
		...
		var tag = {
			...
			//滑动验证码方法
			verify: function(arr, box, imgbox, btn, img, que, slider, btit,tu,ming,that) {
			...
			Math.random() * (imgbox.offsetWidth - img.offsetWidth - imgbox.offsetWidth / 2) + imgbox.offsetWidth /2;
				...
				box.onmouseup = function() {
					...
					if (yes < 10 && yes > -10) {
						...
						img.style.opacity = "0";
						que.style.opacity = "0";
						var bag=document.getElementById("box-bag")
		...
		function imgver(data){
			var that=data
			var bigbox=document.createElement("div");
			bigbox.id="box-bag";
			bigbox.innerHTML='<div id="box"><h3 class="btit">请完成图片验证</h3>'+
			'<div class="img">'+'<img src="img/t1.png" alt="..." id="tu">'+
			'<div class="kuai"></div>'+'<div class="kuai2"></div></div>'+
			'<div class="slider"><div class="ming"></div><button type="button" id="btn"></button></div></div>'
			...
			var ming=query(".ming");
			return tag.verify(arr, box, imgbox, btn, img, que, slider, btit,tu,ming,that)
		}